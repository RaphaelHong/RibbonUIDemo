...
BOOL CEthernetPort::OpenSocket()
{
	...
	if(!bSockErr && (::setsockopt(m_sockClient,SOL_SOCKET,SO_SNDTIMEO,(char *)&TimeOut,sizeof(TimeOut))==SOCKET_ERROR))
		bSockErr = TRUE;

	TimeOut=200;//设置接收超时1秒
	if(!bSockErr && (::setsockopt(m_sockClient,SOL_SOCKET,SO_RCVTIMEO,(char *)&TimeOut,sizeof(TimeOut))==SOCKET_ERROR))
		bSockErr = TRUE;
