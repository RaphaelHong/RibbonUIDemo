...
void CSceneManagerDlg::PreShowWindow()
{
	...
	if (m_nManagerType == SCENE_MANAGER_SAVE)
	{
		...
		SetDlgItemText(IDOK, "读取(&L)");
	else if (m_nManagerType == SCENE_MANAGER_EDIT)
		SetDlgItemText(IDOK, "更新(&R)");
}
...
void CSceneManagerDlg::SaveScene()
{
	...
	if (GetSceneManager()->FindItemByID(nSaveID) != NULL)
	{
		...
		// 发送场景保存命令
		nRet = GetCmdManager()->CmdSAVE(SYSCARD_PORT, eCardTypeCtrl, nSaveID);
	}
	else
	{
		// 发送场景保存命令
...
void CSceneManagerDlg::LoadScene()
{
	...
	if (GetSceneManager()->LoadSceneFromDevice(m_nSceneID))
		AfxMessageBox(_T("已更新场景！"), MB_OK|MB_ICONINFORMATION);
	else
		AfxMessageBox(_T("Err, 更新场景失败！"), MB_OK|MB_ICONERROR);
