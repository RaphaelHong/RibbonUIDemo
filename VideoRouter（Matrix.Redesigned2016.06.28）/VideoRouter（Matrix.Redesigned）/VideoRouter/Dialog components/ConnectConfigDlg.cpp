...
void CConnectConfigDlg::OnBnClickedBtnNetDetect()
{
	...
	if (GetCmdManager()->CmdRNET(SYSCARD_PORT, eCardTypeCtrl, np) != CMD_RTOK)
	{
...
void CConnectConfigDlg::OnBnClickedBtnNetApply()
{
	...
	if (GetCmdManager()->CmdWNET(SYSCARD_PORT, eCardTypeCtrl, np) != CMD_RTOK)
		MessageBox(_T("错误，设置失败！"), _T("网络设置"), MB_OK|MB_ICONERROR);
	else
		MessageBox(_T("设置成功！"), _T("网络设置"), MB_OK|MB_ICONINFORMATION);
}
...
{
	//厂商版本号
	if (GetCmdManager()->CmdRMVR(SYSCARD_PORT, eCardTypeCtrl) != 1408)
	{
		return FALSE;
	}

	//机器ID号
	//if (GetCmdManager()->CmdRDID(SYSCARD_PORT, eCardTypeCtrl) != SYSCARD_PORT)
	//{
	//	return FALSE;
	//}

	//机器功能码
	int nDevCode = GetCmdManager()->CmdRDFN(SYSCARD_PORT, eCardTypeCtrl);

	if ((nDevCode & 0xFF000000) != 0xA0000000)
		return FALSE;
	else
		return TRUE;
