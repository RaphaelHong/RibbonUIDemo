...
void COutputConfigDlg::InitOutputList()
{
	...
	for (int i=0; i<nOutNum; i++)
	{
		...
		m_listOutput.InsertItem(LVIF_TEXT, nItem, strItem, 0, LVIS_SELECTED, 0, 0);

		// 名称
		strItem.Format("%s", GetOutputManager()->GetName(i, FALSE));
		m_listOutput.SetItemText(nItem, 1, strItem);
		...
		m_listOutput.SetItemText(nItem, 2, m_ResManager.GetResolutionName(st));
	}
...
void COutputConfigDlg::OnNMClickListOutput(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	TIMING st = {0};
	st = GetOutputManager()->GetTiming(GetOutputManager()->RealIndex2LogicIndex(pNMItemActivate->iItem), FALSE);
...
void COutputConfigDlg::ApplySetting()
{
	...
	for (int i=0; i<nItemCount; i++)
	{
		if (m_listOutput.GetCheck(i))
		{
			...
			nLogicIndex = GetOutputManager()->RealIndex2LogicIndex(i);
			//发送命令
			nRet = GetCmdManager()->CmdWOUT(GetOutputManager()->GetCmdPort(nLogicIndex, FALSE), eCardTypeOutput, 0, st);
			if (nRet == CMD_FAILURE)				
				UpState[nSelectedCount].val = 0;
			else
			{
				UpState[nSelectedCount].val = 1;
				GetOutputManager()->SetTiming(nLogicIndex, st, FALSE);
			}
...
void COutputConfigDlg::OnBnClickedBtnGetSlaveTiming()
{
	...
	TIMING st = {0};
	int nLogicIndex = GetOutputManager()->RealIndex2LogicIndex(nItem);
	if (GetCmdManager()->CmdREDN(GetOutputManager()->GetCmdPort(nLogicIndex, FALSE), eCardTypeOutput, 0, st) != CMD_RTOK)
	{
...
void COutputConfigDlg::DeleteResolution(const TIMING &timing)
{
	...
	int nLogicIndex = GetOutputManager()->RealIndex2LogicIndex(nItem);
	st = GetOutputManager()->GetTiming(nLogicIndex, FALSE);
...
void COutputConfigDlg::OnNMRClickListOutput(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	CString str;
	str.Format(_T("%s"), GetOutputManager()->GetName(nLogicIndex, FALSE));

	CRenameDlg DLG(str, this);
	if (DLG.DoModal() == IDOK)
	{
		GetOutputManager()->SetName(nLogicIndex, DLG.m_strOrgName.GetBuffer(), FALSE);
		//str.Format(_T("<%03d> "), nLogicIndex+1);
		str = DLG.m_strOrgName;
		m_listOutput.SetItemText(nLogicIndex, 1, str.GetBuffer());
	}
...
void COutputConfigDlg::InitChannelTiming()
{
	...
	for (int i=0; i<GetOutputManager()->GetCount(); i++)
	{
		...
		{
			GetCmdManager()->CmdROUT(GetOutputManager()->GetCmdPort(i, FALSE), eCardTypeOutput, 0, st);
...
void COutputConfigDlg::OnBtnReset()
{
	...
	for (int i=0; i<nItemCount; i++)
	{
		if (m_listOutput.GetCheck(i))
		{
			...
			//发送命令
			nLogicIndex = GetOutputManager()->RealIndex2LogicIndex(i);
			nRet = GetCmdManager()->CmdRSTF(GetOutputManager()->GetCmdPort(nLogicIndex, FALSE), eCardTypeInput);
