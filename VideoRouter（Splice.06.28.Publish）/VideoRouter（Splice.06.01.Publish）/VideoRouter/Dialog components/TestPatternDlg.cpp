...
void CTestPatternDlg::InitListCtrl()
{
	...
	nCount = GetOutputManager()->GetCount();
	for (int i=0; i<nCount; i++)
	{
...
void CTestPatternDlg::OnBtnTest()
{
	...
	for (int i=0; i<pList->GetItemCount(); i++)
	{
		if (pList->GetCheck(i))
		{
			...
			nRet = GetCmdManager()->CmdImageTest(nCmdPort, m_Tab.GetCurSel()==0 ? eBoardTypeInput:eBoardTypeOutput, &tstinfo);
...
void CTestPatternDlg::OnBnClickedBtnPatternPrintOsd()
{
	...
	for (int i=0; i<pList->GetItemCount(); i++)
	{
		if (pList->GetCheck(i))
		{
			...
			nRet = GetCmdManager()->CmdYOSD(nCmdPort, m_Tab.GetCurSel()==0 ? eBoardTypeInput:eBoardTypeOutput);
...
void CTestPatternDlg::OnBnClickedBtnPatternCancelOsd()
{
	...
	for (int i=0; i<pList->GetItemCount(); i++)
	{
		if (pList->GetCheck(i))
		{
			...
			nRet = GetCmdManager()->CmdCOSD(nCmdPort, m_Tab.GetCurSel()==0 ? eBoardTypeInput:eBoardTypeOutput);
