...
void CInputConfigDlg::ApplySetting()
{
	...
	for (int i=0; i<nItemCount; i++)
	{
		if (m_listInput.GetCheck(i))
		{
			...
			//发送命令
			nRet = GetCmdManager()->CmdWINT(GetInputManager()->GetCmdPort(i, FALSE), eBoardTypeInput, 0, st);
...
void CInputConfigDlg::DeleteResolution(const TIMING &timing)
{
	...
	st = GetInputManager()->GetTiming(nItem, FALSE);
...
{	
	...
	for (int i=0; i<GetInputManager()->GetCount(); i++)
	{
		...
		{
			GetCmdManager()->CmdRINT(GetInputManager()->GetCmdPort(i,FALSE), eBoardTypeInput, 0, st);
...
void CInputConfigDlg::OnBtnReset()
{
	...
	for (int i=0; i<nItemCount; i++)
	{
		if (m_listInput.GetCheck(i))
		{
			...
			//发送命令
			nRet = GetCmdManager()->CmdRSTF(GetInputManager()->GetCmdPort(i, FALSE), eBoardTypeInput);
