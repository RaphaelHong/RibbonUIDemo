...
BOOL CInputManager::UpdateEDID(const int iKey, BOOL byID) const
{
	...
	if (OpenFile.DoModal() == IDOK)
	{
		...
		if (GetCmdManager()->CmdWEDI(GetInputManager()->GetCmdPort(iKey, byID), eBoardTypeInput, Allocate_UINT.GetAddress(), nDataLen) == CMD_FAILURE)
		{
...
{
	const int iDemoInputs = 24;//_MAX_INPUT_PORT;
...
BOOL CInputManager::RealInit()
{
	...
	GetMainFrame()->ResetSourceTreeItem();
	GetMainFrame()->ResetScreenTreeItem();
...
BOOL CInputManager::InitPorts()
{
	...
	int nRet = GetCmdManager()->CmdGCMG(SYSBOARD_PORT, eBoardTypeCtrl, nInputs, nOutputs, InputFlags, OutputFlags);
	if ( nRet == CMD_FAILURE )
	{
	...
	for (int i=0; i<iOutNum; i++)
	{
		...
		//if ((OutputPorts[i] & 0xFF0000) == 0xFF0000)
		//{
		//	sOut.m_nSubPort = 2;
		//	sOut.m_nCmdPort  = (OutputPorts[i] & 0xFFFF);
		//}
		//else
		//{
		//	sOut.m_nSubPort = 1;
...
BOOL CInputManager::InitSignals()
{
	...
	{
		if (GetCmdManager()->CmdRPM3(GetCmdPort(i, FALSE), eBoardTypeInput, nVideoType) != CMD_FAILURE)
...
BOOL CInputManager::RefreshThisSignal(const int nIndex)
{
	...
	// 当前输入信号类型
	int nVideoType = vtUndef;
	if (GetCmdManager()->CmdRPM3(GetCmdPort(nIndex, FALSE), eBoardTypeInput, nVideoType) != CMD_FAILURE)
	...
	// 当前输入信号分辨率
	TIMING timing = {0};
	if (GetCmdManager()->CmdGETR(GetCmdPort(nIndex, FALSE), eBoardTypeInput, timing) != CMD_FAILURE)
...
BOOL CInputManager::Reset(const int iKey, BOOL byID/*=TRUE*/)
{
	...
	return GetCmdManager()->CmdRSTF(m_arrPort[nIndex].m_nCmdPort, eBoardTypeInput);
}
...
BOOL CInputManager::ResetAll()
{
	for (int i=0; i<m_arrPort.GetSize(); i++)
	{
		...
		GetCmdManager()->CmdRSTF(m_arrPort[i].m_nCmdPort, eBoardTypeInput);
	}
