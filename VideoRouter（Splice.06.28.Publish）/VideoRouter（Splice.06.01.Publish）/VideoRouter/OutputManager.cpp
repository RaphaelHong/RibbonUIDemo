...
//
//	// 发送切换命令
//	int nRet = GetCmdManager()->CmdSWCH(SYSBOARD_PORT, eBoardTypeCtrl, iFeederPort, iSubPort, iOutChlPort);
//
//#ifndef _DEBUG
//	if (nRet == CMD_FAILURE)
...
//
//	// 切换命令
//	int nRet = GetCmdManager()->CmdSWCH(SYSBOARD_PORT, eBoardTypeCtrl, iFeederPort, iSubPort);
//
//#ifndef _DEBUG
//	if (nRet == CMD_FAILURE)
//		return (void)AfxMessageBox(_T("矩阵切换失败！"), MB_OK|MB_ICONERROR);
...
//	}
//
//	int nRet = GetCmdManager()->CmdSWCH(SYSBOARD_PORT, eBoardTypeCtrl, iFeederPort, iSubPort, pOutChlPort, iCount);
//
//#ifndef _DEBUG
//	if (nRet == CMD_FAILURE)
//		return (void)AfxMessageBox(_T("矩阵切换失败！"), MB_OK|MB_ICONERROR);
...
//	}
//
//	int nRet = GetCmdManager()->CmdSWCH(SYSBOARD_PORT, eBoardTypeCtrl, iMax, FeederArr.GetAddress());
//
//#ifndef _DEBUG
//	if (nRet == CMD_FAILURE)
//		return (void)AfxMessageBox(_T("矩阵切换失败！"), MB_OK|MB_ICONERROR);
...
{
	const int iDemoOutputs = 20;//_MAX_OUTPUT_PORT;
...
BOOL COutputManager::Reset(const int iKey, BOOL byID/*=TRUE*/)
{
	...
	return GetCmdManager()->CmdRSTF(m_arrPort[nIndex].m_nCmdPort, eBoardTypeOutput);
}
...
BOOL COutputManager::ResetAll()
{
	for (int i=0; i<m_arrPort.GetSize(); i++)
	{
		...
		GetCmdManager()->CmdRSTF(m_arrPort[i].m_nCmdPort, eBoardTypeOutput);
	}
