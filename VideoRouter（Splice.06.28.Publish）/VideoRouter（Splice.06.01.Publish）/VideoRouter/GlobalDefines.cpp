...
BOARDINFO stBoardInfo[_MAX_BOARD_DEF] = {
	{ eMM10X_SYS_CTRL,  eBoardTypeCtrl,   0 },
	{ eMM10X_4IN_DVIM,  eBoardTypeInput,  4 },
	{ eMM10X_2IN_DPDVI, eBoardTypeInput,  2 },
	{ eMM10X_4OUT_DVII, eBoardTypeOutput, 4 }
};

emBoardType GetBoardType(const UINT uBoardID)
{
	for (int i=0; i<_MAX_BOARD_DEF; i++) {
		if (stBoardInfo[i].m_eBoardID == uBoardID)
		{
			return stBoardInfo[i].m_eBoardType;
		}
	}

	return eBoardTypeUndef;
}
