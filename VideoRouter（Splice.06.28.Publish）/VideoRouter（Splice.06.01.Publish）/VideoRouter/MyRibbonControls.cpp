...
void COutlookBar::OnContextMenu(CWnd* pWnd, CPoint point)
{
	...
	CWnd* pActWnd = pOutlookBar->GetActiveWnd();
	pActWnd->SendMessage(WM_CONTEXTMENU);

	return;
}

BOOL COutlookBar::OnShowControlBarMenu(CPoint point)
{
	return FALSE;
}
...
/*	
...

	{
		...
		if (m_bIsCustomizeMenu)
		{
			...
			//menu.AppendMenu(MF_STRING, uiMenuCmd, _T("ÍË³ö"));
